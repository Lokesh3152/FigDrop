...
function FigDropWidget() {
    const [roomCode, setRoomCode] = useSyncedState('roomCode', () => 'DROP-1000');
    const [pendingCount, setPendingCount] = useSyncedState('pendingCount', 0);
    ...
    return (figma.widget.h(AutoLayout, { direction: "vertical", padding: 18, cornerRadius: 6, fill: "#FFFFFF", stroke: "#E5E5E5", strokeWidth: 1, spacing: 12, width: 290 },
        figma.widget.h(AutoLayout, { direction: "horizontal", width: "fill-parent", verticalAlignItems: "center", spacing: "auto" },
            figma.widget.h(Text, { fontSize: 14, fontWeight: "bold", fill: "#1E1E1E" }, "FigDrop Collector"),
            figma.widget.h(AutoLayout, { direction: "horizontal", padding: { vertical: 2, horizontal: 6 }, cornerRadius: 4, fill: pendingCount > 0 ? '#F0ECFF' : '#F5F5F5', stroke: pendingCount > 0 ? '#D6CCFF' : '#E5E5E5', strokeWidth: 1 },
                figma.widget.h(Text, { fontSize: 11, fontWeight: "bold", fill: pendingCount > 0 ? '#7B61FF' : '#666666' }, roomCode))),
        figma.widget.h(Text, { fontSize: 12, fill: "#757575" }, pendingCount > 0
            ? `${pendingCount} capture${pendingCount > 1 ? 's' : ''} waiting on room ${roomCode}.`
            : `Pair extension with code ${roomCode} to collect references.`),
        figma.widget.h(AutoLayout, { padding: { vertical: 10, horizontal: 14 }, cornerRadius: 6, fill: "#7B61FF", hoverStyle: { fill: '#6949FF' }, horizontalAlignItems: "center", verticalAlignItems: "center", width: "fill-parent", onClick: openCollector },
            figma.widget.h(Text, { fontSize: 13, fontWeight: "bold", fill: "#FFFFFF" }, pendingCount > 0 ? `⚡ Drop ${pendingCount} Images` : 'Open FigDrop'))));
}
